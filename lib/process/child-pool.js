...
ChildPool.prototype.kill = function(child, signal) {
  this.remove(child);
  return killAsync(child, signal || 'SIGKILL', CHILD_KILL_TIMEOUT).then(() => {
    psTree(child.pid, function (err, childrens) {
      if (!err) {
        spawn('kill', ['-9'].concat(childrens.map(function (p) { return p.PID })));
      }
      return child;
    });
  });
